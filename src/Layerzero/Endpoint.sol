...
contract Endpoint is Ownable, ILayerZeroEndpoint {
    ...
    function send(
        ...
        _getSendLibrary(uaConfig).send{value: msg.value}(
            msg.sender, nonce, _dstChainId, _destination, _payload, _refundAddress, _zroPaymentAddress, _adapterParams
        );
    }
    ...
    function setDefaultSendVersion(uint16 _newDefaultSendVersion)
        external
        onlyOwner
        validVersion(_newDefaultSendVersion)
    {
    ...
    function setDefaultReceiveVersion(uint16 _newDefaultReceiveVersion)
        external
        onlyOwner
        validVersion(_newDefaultReceiveVersion)
    {
        ...
        validVersion(_version)
    {
        if (_version == DEFAULT_VERSION) {
            require(
                defaultSendVersion == defaultReceiveVersion, "LayerZero: can not set Config during DEFAULT migration"
            );
    ...
    function estimateFees(
        ...
        ILayerZeroMessagingLibrary lib =
            uaConfig.sendVersion == DEFAULT_VERSION ? defaultSendLibrary : uaConfig.sendLibrary;
    ...
    function getSendLibraryAddress(address _userApplication)
        ...
        view
        override
        returns (address sendLibraryAddress)
    {
        LibraryConfig storage uaConfig = uaConfigLookup[_userApplication];
    ...
    function getReceiveLibraryAddress(address _userApplication)
        ...
        view
        override
        returns (address receiveLibraryAddress)
    {
        LibraryConfig storage uaConfig = uaConfigLookup[_userApplication];
